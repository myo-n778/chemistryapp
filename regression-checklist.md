...
## 質問ログ（2026-09-16）
- 利用者向けにAI/OpenAIの語が出ない。
- 定型操作・自由質問の表示成功後だけ5列を保存し、問題と解答はサーバー正本を使う。
- 氏名は生成要求に含めない。数式風の入力も文字列。日時はJST。
- 記録失敗で回答を失わず、手動の記録再送で重複行・追加生成・表示回数増加がない。
- 期限切れ・未完了回答・不正token/requestIdでは記録しない。
- 既存分野別回数・Next・採点・成績保存は維持。
