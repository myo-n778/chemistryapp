...
const errorMessages:Record<string,string>={
 not_configured:'解説は準備中です。通常の演習は続けられます。',
 session_limit:'解説の利用開始が集中しています。少し待ってから再試行してください。',
 material_mismatch:'問題と解説サービス側の教材が一致しないため、説明を止めました。通常の解説を使ってください。',
 session_expired:'解説の利用セッションが切れました。出題タイプへ戻って学習を開始し直してください。',
 daily_limit:'今日の解説利用上限に達しました。通常の演習は続けられます。',
 usage_limit:'解説の利用上限に達しました。通常の演習は続けられます。',
 insufficient_quota:'解説サービス側の利用枠が不足しています。管理者による確認が必要です。',
 rate_limit_exceeded:'解説サービス側が混み合っています。通常の演習を続け、必要なときに再試行してください。',
 request_busy:'別の解説要求を処理中です。少し待ってから再試行してください。',
...
export const tutorErrorMessage = (error:unknown) => error instanceof TutorClientError
 ? errorMessages[error.code] || '解説を取得できませんでした。通常の解説で学習を続けられます。'
 : '解説サービスに接続できませんでした。通常の解説で学習を続けられます。';
async function post(path:string, body:unknown, signal:AbortSignal, session?:TutorSession) {
 const gas = !!AI_TUTOR_GAS_URL;
 const response=await fetch(gas ? AI_TUTOR_GAS_URL : path,{method:'POST',signal,credentials:'omit',headers:gas ? {'Content-Type':'text/plain;charset=UTF-8'} : {'Content-Type':'application/json',...(session?{'X-Chem-Session':session.token}:{})},body:JSON.stringify(gas ? {...(body as object),operation:path.split('/').pop(),...(session?{token:session.token}:{})} : body),cache:'no-store'});
...
export async function requestTutorReply(q:TutorQuestion,action:TutorAction,text:string,signal:AbortSignal,session:TutorSession):Promise<{reply:TutorReply;budget:TutorBudget;requestId:string}> {
 ...
 return {reply:data.reply,budget:data.budget,requestId};
}

export const supportsTutorLogging = () => !!AI_TUTOR_GAS_URL;
export async function logTutorDisplay(requestId:string,username:string,session:TutorSession):Promise<void> {
 const controller=new AbortController();
 const timer=setTimeout(()=>controller.abort(),30000);
 try {
  const data=await post('/api/tutor/log',{requestId,username},controller.signal,session);
  if(data.logged!==true)throw new TutorClientError('log_failed');
