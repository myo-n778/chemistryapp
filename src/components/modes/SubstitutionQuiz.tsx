...
export const SubstitutionQuiz: React.FC<SubstitutionQuizProps> = ({ compounds, category, onBack, isShuffleMode = false, quizSettings, totalCount: _totalCount = 0, onNextRange }) => {
  ...
  useEffect(() => {
    if (isFinished && totalAnswered > 0) {
      console.log('[SubstitutionQuiz] isFinished changed to true, playing finish sound');
      const getModeAndRangeKey = () => {
        const mode = `substitution-${category}`;
        const rangeKey = quizSettings?.questionCountMode && quizSettings.questionCountMode === 'batch-10' 
          ? getRangeKey('batch-10', quizSettings.startIndex)
      ...
      console.log(`[SubstitutionQuiz] Result: score=${score}/${totalAnswered} (${percentage}%), isNewRecord=${isNewRecord}, isRankIn=${isRankIn}`);
      if (isNewRecord) {
        console.log('[SubstitutionQuiz] Playing finish sound: type 3 (最高記録更新)');
        playFinishSound(3); // 最高記録更新
      } else if (isRankIn) {
        console.log('[SubstitutionQuiz] Playing finish sound: type 2 (ランクイン)');
        playFinishSound(2); // ランクイン
      } else if (percentage === 100) {
        console.log('[SubstitutionQuiz] Playing finish sound: type 5 (満点)');
        playFinishSound(5); // 満点で記録更新でない
      } else if (percentage >= 60) {
        console.log('[SubstitutionQuiz] Playing finish sound: type 1 (60%以上)');
        playFinishSound(1); // 60%以上
      } else {
        console.log('[SubstitutionQuiz] Playing finish sound: type 4 (60%未満)');
    ...
    <div className="quiz-container" style={{ cursor: showResult ? 'pointer' : 'default' }} onClick={handleContentClick} onTouchEnd={handleTouchEnd}>
      <div className="quiz-header">
        ...
        <div className="reaction-quiz-wrapper">
          ...
          {showResult && <AiTutorPanel question={{ id: currentReaction.questionId || `${currentReaction.from}-${currentReaction.to}-${currentReaction.reagent}`, mode: 'substitution', category, prompt: `${currentReaction.from}から${currentReaction.to}：何をした？`, correct: currentReaction.reagent, selected: selectedAnswer || '', explanation: currentReaction.learningPoint,  }} />}
