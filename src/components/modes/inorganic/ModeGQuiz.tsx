 ...
 * 判定：tags_norm / family
 */
export const ModeGQuiz: React.FC<ModeGQuizProps> = ({
  reactions,
  category,
  onBack,
  isShuffleMode = false,
  ...
  const [questionType, setQuestionType] = useState<'tags' | 'observations'>('tags');
  const [choiceType, setChoiceType] = useState<'equation' | 'reactants'>('equation');
  const isProcessingRef = useRef(false);

  const filteredReactions = useMemo(() => {
    let filtered = reactions.filter(r => {
      if (questionType === 'tags') {
        return r.tags_norm && r.tags_norm.length > 0;
      } else {
        return r.observations && r.observations.trim() !== '';
      }
    });
    ...
    return filtered;
  }, [reactions, quizSettings, isShuffleMode, questionType]);
  ...
  useEffect(() => {
    // 問題タイプと選択肢タイプをランダムに選択
    setQuestionType(Math.random() > 0.5 ? 'tags' : 'observations');
    setChoiceType(Math.random() > 0.5 ? 'equation' : 'reactants');
  }, [currentIndex]);

  // 選択肢を生成（同tags_normまたは同familyから）
  const choices = useMemo(() => {
    if (!currentReaction) return { choices: [], correctIndex: 0 };
    
    const correctAnswer = choiceType === 'equation' 
      ? (currentReaction.equation_tex_mhchem || currentReaction.equation_tex)
      : currentReaction.reactants_desc;

    // 同tags_normまたは同familyから誤答を生成
    const sameTagsOrFamily = reactions.filter(r => {
      if (r.id === currentReaction.id) return false;
      if (questionType === 'tags') {
        return r.tags_norm.some(tag => currentReaction.tags_norm.includes(tag));
      } else {
        return r.family === currentReaction.family || 
               r.tags_norm.some(tag => currentReaction.tags_norm.includes(tag));
      }
    });

    const distractors = sameTagsOrFamily
      .map(r => choiceType === 'equation' 
        ? (r.equation_tex_mhchem || r.equation_tex)
        : r.reactants_desc)
      .filter((choice, index, self) => choice && choice !== correctAnswer && self.indexOf(choice) === index)
      .slice(0, 3);

    // 不足する場合はランダムに追加
    while (distractors.length < 3) {
      const randomReaction = reactions[Math.floor(Math.random() * reactions.length)];
      if (randomReaction.id !== currentReaction.id) {
        const choice = choiceType === 'equation'
          ? (randomReaction.equation_tex_mhchem || randomReaction.equation_tex)
          : randomReaction.reactants_desc;
        if (choice && choice !== correctAnswer && !distractors.includes(choice)) {
  ...
  }, [currentReaction, reactions, questionType, choiceType]);
  ...
  const handleNext = () => {
    ...
    const getModeAndRangeKey = () => {
      const mode = `inorganic-mode-g-${category}`;
  ...
  if (!currentReaction && !isFinished) {
    return (
      <div className="quiz-container">
        <div className="quiz-header"><h1>Inorganic Chemistry Drill - Mode G</h1></div>
  ...
  return (
    <div className="quiz-container">
      <div className="quiz-header">
        <h1>Inorganic Chemistry Drill - Mode G</h1>
        <ScoreDisplay
          score={score}
          totalAnswered={totalAnswered}
          pointScore={pointScore}
          showPoints={true}
          mode={`inorganic-mode-g-${category}`}
      ...
      <div className="quiz-content">
        <div className="question-area">
          ...
          {questionType === 'tags' && currentReaction.tags_norm && currentReaction.tags_norm.length > 0 && (
            <div className="visual-area">
              <InorganicVisualViewer reaction={currentReaction} width={400} height={300} />
            </div>
          )}
          {questionType === 'observations' && (
          ...
          {choices.choices.map((choice, index) => {
            ...
            return (
              <button
                ...
                disabled={showResult}
              >
                {choiceType === 'equation' && choice.includes('\\ce{') ? (
        ...
        {showResult && (
          <div className="result-panel">
            ...
            <div className="result-explanation">
              <p><strong>正解:</strong> {choiceType === 'equation' ? (
                <TeXRenderer equation={currentReaction.equation_tex_mhchem || currentReaction.equation_tex} displayMode={true} />
              ) : (
                currentReaction.reactants_desc
              )}</p>
