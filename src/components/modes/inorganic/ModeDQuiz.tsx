 ...
 */
export const ModeDQuiz: React.FC<ModeDQuizProps> = ({
  reactions,
  category,
  onBack,
  isShuffleMode = false,
  ...
  const [subMode, setSubMode] = useState<'d1' | 'd2' | 'd3'>('d1');
  const isProcessingRef = useRef(false);

  const filteredReactions = useMemo(() => {
    let filtered = reactions.filter(r => r.tags_norm && r.tags_norm.length > 0);
  ...
  // サブモードをランダムに選択（または順番に）
  useEffect(() => {
    const modes: ('d1' | 'd2' | 'd3')[] = ['d1', 'd2', 'd3'];
  ...
  const choices = useMemo(() => {
    ...
    let answerField: 'products_desc' | 'equation_tex' | 'observations';

    if (subMode === 'd1') {
      correctAnswer = currentReaction.products_desc;
      answerField = 'products_desc';
    } else if (subMode === 'd2') {
      correctAnswer = currentReaction.equation_tex_mhchem || currentReaction.equation_tex;
      answerField = 'equation_tex';
    } else {
  ...
  const handleNext = () => {
    ...
    const getModeAndRangeKey = () => {
      const mode = `inorganic-mode-d-${category}`;
  ...
  if (!currentReaction && !isFinished) {
    return (
      <div className="quiz-container">
        <div className="quiz-header"><h1>Inorganic Chemistry Drill - Mode D</h1></div>
  ...
  const getQuestionTitle = () => {
    if (subMode === 'd1') return 'この図から判断される生成物質は？';
    if (subMode === 'd2') return 'この図に対応する反応式は？';
    return 'この図から判断される観察は？';
  };

  return (
    <div className="quiz-container">
      <div className="quiz-header">
        <h1>Inorganic Chemistry Drill - Mode D</h1>
        <ScoreDisplay
          score={score}
          totalAnswered={totalAnswered}
          pointScore={pointScore}
          showPoints={true}
          mode={`inorganic-mode-d-${category}`}
