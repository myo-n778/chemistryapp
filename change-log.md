...
## 2026-09-13 — AI学習支援の操作プレビュー（ローカル、未公開）

設計の承認後、模擬APIで画面実装する依頼に対応。AiTutorPanel/AiTutorSessionとaiTutorMockを追加し、有機6・無機4タイプへ接続。既存のLearningPointと無機解説は維持した。入力欄にEnter/スペースやタップが伝播すると既存の画面全体Next操作を起動しうるため、質問欄内のイベントだけ止めた。採点・保存・GAS・Sheets・教材本文は変更していない。

全10タイプ×3幅のUI、回数・文字数上限、エラー、30秒タイムアウト、遅延応答、連打、配布ビルドで無効を模擬検証。TS/Vite・差分と画像目視を確認。実APIコード・APIキー・実費用・公開は未着手。Company WI-20260913-123957-9e7d46。
